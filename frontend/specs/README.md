# MVP v1 — Specs Modulares (fase SPECIFY → DESIGN)
...
Os 3 módulos de frontend (`07`, `08`, `09`) já têm `spec.md` (fase SPECIFY) e
`design.md` (fase DESIGN) escritos. Ver `decisoes-tecnicas.md` para as decisões
técnicas transversais aos 3 módulos (data fetching, formulários, autenticação).
`tasks.md` (quebra em tarefas executáveis) é o próximo passo, módulo a módulo.

**Gap identificado na fase Design:** os módulos `07-frontend-autenticacao` e
`08-frontend-gestao-clientes` dependem de endpoints de backend que ainda não
existem (login/auth, CRUD de clientes). Ver a seção "Gaps de API" de cada
`design.md`. `09-frontend-dashboard-cobrancas` é o único que já pode iniciar
execução da listagem principal sem esperar novo backend (o endpoint de
detalhe da cobrança ainda falta).
