...
- **EMAIL-R-02**: WHEN o sistema precisa enviar um e-mail, THEN o sistema SHALL autenticar via Gmail API usando OAuth2 (conta Google Workspace/Gmail da empresa), nunca usando usuário/senha em texto puro.
...
### Decisão técnica
- Envio via **Gmail API** (não SMTP simples), autenticado por OAuth2, permitindo enviar em nome da conta Gmail configurada sem expor senha
...
|---|---|---|---|
| EMAIL-01 | EMAIL-US-01 | EMAIL-R-01, EMAIL-R-03 | `CanalNotificacao` (porta), `GmailNotificador` (adapter) |
| EMAIL-02 | EMAIL-US-04 | EMAIL-R-02 | OAuth2 Gmail (credenciais em variável de ambiente/secret) |
